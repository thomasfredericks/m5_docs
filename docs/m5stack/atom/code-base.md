...
unsigned long interactionChrono; // Utilisé dans loop() plus bas pour limiter la vitesse de l'interaction
...
void loop() {
  ...
  if ( millis() - interactionChrono >= 20 ) { // Si 20 millisecondes se sont écoulées
    interactionChrono = millis(); // Repartir le compteur
    // Ajouter interaction ici
    // Ajouter interaction ici
    // Ajouter interaction ici
  }
